...
class _CardSortGameState extends State<CardSortGameScreen> with TickerProviderStateMixin, FeedbackMixin {
  ...
  void _restart() {
    setState(() { _score = 0; _lives = 3; _resetCards(); });
    _next();
  }

  void _next() {
    final target = _rnd.nextInt(6) + 1;
    setState(() {
      _target = target;
      _locked = false;
      _cards.shuffle();
      ...
      child: Stack(children: [
        Padding(
          ...
          child: Column(children: [
            Text('اسحب البطاقة التي تحتوي على $_target', style: GoogleFonts.cairo(textStyle: TextStyle(fontSize: SizeConfig.sp(18)))),
            SizedBox(height: 2.h),
            Wrap(spacing: 2.w, children: _cards.map((c) {
              return Draggable<int>(
                data: c.value,
                feedback: Material(child: _buildCard(c, flipped: true)),
                childWhenDragging: Opacity(opacity: 0.4, child: _buildCard(c)),
                child: _buildCard(c),
              );
            }).toList()),
            SizedBox(height: 3.h),
            DragTarget<int>(onAccept: _onDrop, builder: (context, candidate, rejected) {
              return Container(height: 30.h, width: double.infinity, decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(3.w), border: Border.all(color: Colors.grey.shade300)), child: Center(child: Text('ضع البطاقة هنا', style: GoogleFonts.cairo(textStyle: TextStyle(fontSize: SizeConfig.sp(18))))));
            }),
          ]),
        ),
  ...
  Widget _buildCard(_CardSortGameStateData c, {bool flipped = false}) {
    return AnimatedContainer(duration: const Duration(milliseconds: 350), margin: EdgeInsets.all(1.5.w), width: 18.w, height: 18.h, decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(2.w), boxShadow: [BoxShadow(color: Colors.black12, blurRadius: 6, offset: const Offset(0, 3))]), child: Center(child: Text('${c.value}', style: GoogleFonts.cairo(textStyle: TextStyle(fontSize: SizeConfig.sp(20))))));
  }
}
